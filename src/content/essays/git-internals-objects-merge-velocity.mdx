...
The key insight: **this structure is deterministic**. Same tree, same parent, same timestamp, same message means same hash. Commit `abc123` today is identical to commit `abc123` next year on a different computer.
...
Why does this matter? When Git merges two branches, it needs their **common ancestor**. This is the last commit both branches share. Git traverses the DAG backward from both tips until it finds it. That's the "merge base." Git then performs a **three-way merge** using the merge base, main's tip, and feature's tip. We'll explore this next.
...
Now you understand Git from the inside. You know commits are content-addressed hashes, immutable and deterministic. You understand three-way merge looks at what each side changed from a common ancestor, not just comparing before and after. You can explain why merge commits, rebase, and squash each make sense in different situations.

This matters because you'll hit moments where understanding this stuff actually helps. You'll read a merge conflict and know exactly what happened and why. You'll debug production issues and use `git blame` or `git bisect` with confidence because you understand the commit graph. You'll make better decisions about branching strategy because you've thought through the trade-offs.

And when your team debates whether to rebase or merge, you'll have a real framework for the decision. Not cargo-cult rules, but an understanding of what each strategy actually does to your history and how that affects your workflow.

That's the value of understanding Git internals. Not velocity metrics or productivity claims. Just a clearer mental model of a tool you use every single day. When you understand how something works, you use it better.
